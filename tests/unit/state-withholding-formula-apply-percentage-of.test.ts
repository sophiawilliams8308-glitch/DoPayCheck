...
/**
 * `APPLY_PERCENTAGE_OF` — generic over `RATE`-shaped `StateRuleKey`s, like
 * `APPLY_FLAT_RATE` (Task 4O-6R24-4O-6R26).
 *
 * ===========================================================================
 * SCOPE NOTE.
 *
 * `operandRef` validation, the `RATE` shape check, `applicability`/
 * `appliesTo` handling, and rate normalization via `readRate()` are all
 * TRANSFERRED from `APPLY_FLAT_RATE`, disclosed as an analogy rather than an
 * independently rediscovered fact (Task 4O-6R24 §7) — they attach to the
 * shared `RATE` shape, not to either operation's arithmetic.
 *
 * The ONE respect in which this operation differs from `APPLY_FLAT_RATE` is
 * arithmetic: `runningValue × (1 + rate)`, an EXPLICIT OWNER DECISION (Task
 * 4O-6R26-OWNER) — Tasks 4O-6R24/4O-6R25 found no repository evidence
 * resolving whether this operation meant plain multiplication (which would
 * make it a duplicate of `APPLY_FLAT_RATE`) or this gross-up form. The owner
 * selected the gross-up form; it is not re-derived or questioned here.
 * ===========================================================================
 *
 * Fixtures use RESERVED TEST jurisdiction/source ids and no real tax value.
 */

const JURISDICTION_CODE = 'TEST-APPLY-PERCENTAGE-OF';
const JURISDICTION_ID = 'test-apply-percentage-of-jurisdiction-id';
const TAX_YEAR = 2099;
...
describe('APPLY_PERCENTAGE_OF — arithmetic (owner-locked Option B, Task 4O-6R26-OWNER)', () => {
  it('multiplies the running value by (1 + rate) for a DECIMAL_FRACTION rate', () => {
  ...
  it('multiplies the running value by (1 + rate) for a PERCENT rate, converted via readRate()', () => {
    const ruleSet = buildRuleSet({
      [RATE_KEY]: availableEntry(RATE_KEY, rateDetail({ rate: '10', unit: 'PERCENT' })),
    });
    const detail = formulaDetail([step(0, 'APPLY_PERCENTAGE_OF', RATE_KEY)]);
  ...
  it('differs from APPLY_FLAT_RATE for the identical rate and running value', () => {
    ...
    const percentageOfResult = runStateWithholdingFormula(
      formulaDetail([step(0, 'APPLY_PERCENTAGE_OF', RATE_KEY)]),
      ruleSet,
      money('1000'),
      SINGLE,
      {},
...
describe('APPLY_PERCENTAGE_OF — operandRef validation', () => {
...
describe('APPLY_PERCENTAGE_OF — referenced rule missing/unverified/malformed', () => {
...
describe('APPLY_PERCENTAGE_OF — null rate', () => {
...
describe('APPLY_PERCENTAGE_OF — applicability and appliesTo (transferred from APPLY_FLAT_RATE, Task 4O-6R24 §7)', () => {
  it('reports SCENARIO_UNSUPPORTED for applicability = NOT_APPLICABLE, never leaving the value unchanged or COMPONENT_NOT_STATED', () => {
    const ruleSet = buildRuleSet({
...
describe('APPLY_PERCENTAGE_OF — precision', () => {
...
describe('APPLY_PERCENTAGE_OF — sequential accumulator behavior', () => {
