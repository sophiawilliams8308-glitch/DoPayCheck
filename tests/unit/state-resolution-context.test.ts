...
describe('§3.2 — exactly eight top-level fields', () => {
  it('projects exactly the eight fields the specification names', () => {
    const projected = projectResolutionContext(context());
...
describe('§3.4 — exactly three scenario fields', () => {
  it('projects exactly the three the specification names', () => {
    const { scenario } = projectResolutionContext(context());
  ...
  it('carries employerSideRequested directly', () => {
    expect(projectResolutionContext(context()).scenario.employerSideRequested).toBe(true);
    expect(
      projectResolutionContext(context({ includeEmployerTaxes: false })).scenario
...
describe('purity', () => {
  it('returns a deeply equal result for the same input', () => {
    const input = context();
    const first = projectResolutionContext(input);
    const second = projectResolutionContext(input);
  ...
  it('does not mutate its argument', () => {
    ...
    expect(() => projectResolutionContext(input)).not.toThrow();
  ...
  it('returns independent sets per call, so one caller cannot affect another', () => {
    const first = projectResolutionContext(context());
    const second = projectResolutionContext(context());
...
describe('totality', () => {
  ...
  for (const [name, overrides] of edgeCases) {
    it(`is defined for: ${name}`, () => {
      const projected = projectResolutionContext(context(overrides));
      expect(Object.keys(projected).length).toBe(8);
  ...
  it('represents an absent work jurisdiction as null, never a default', () => {
    ...
    expect(
      projectResolutionContext(context({ workJurisdictions: [] })).workJurisdiction,
    ).toBeNull();
  });

  it('represents a missing filing election as null', () => {
    expect(projectResolutionContext(context({ elections: {} })).stateFilingStatus).toBeNull();
  });
});

describe('deferred §3.3 derivations are empty, not guessed', () => {
  it('emits an empty capability set rather than inventing a derivation', () => {
    // §3.3 does not state how capabilitiesRequested is derived. Empty means
    // "undetermined", NOT "none requested" — a consumer must refuse, not resolve
    // nothing. Asserted so a later step cannot quietly start guessing.
    const { capabilitiesRequested } = projectResolutionContext(context()).scenario;
    expect(capabilitiesRequested.size).toBe(0);
  });

  it('does not pretend all 13 capabilities were requested', () => {
    const { capabilitiesRequested } = projectResolutionContext(context()).scenario;
  ...
  it('emits the same empty sets whatever the wages are', () => {
    const withWages = projectResolutionContext(context()).scenario;
