 ...
 * entirely by Task 4O-6R31). `DEANNUALIZE` is a SEPARATE, unresolved
 * operation (Task 4O-6R33 §5 leaves its own rounding-policy precision
 * question open) and is not implemented or tested here.
 * ===========================================================================
 *
...
describe('ANNUALIZE — DEANNUALIZE and ROUND remain unsupported', () => {
  it("reports METHOD_NOT_IMPLEMENTED for DEANNUALIZE, never sharing ANNUALIZE's contract", () => {
    const ruleSet = buildRuleSet({
      [PERIODS_KEY]: availableEntry(PERIODS_KEY, COUNTS_DETAIL),
    });
